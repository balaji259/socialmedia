...
const GroupAnnouncements = () => {
  ...
  const fetchAnnouncements = async () => {
    try {
      ...
      const res = await axios.get(`/group/get/announcements/${communityId}`, {
  ...
  const checkIfAdmin = async () => {
    ...
    try {
      ...
      const res = await axios.get(`/group/${communityId}`);
  ...
  const handlePost = async () => {
    ...
    try {
      await axios.post(
        `/group/post/announcements/${communityId}`,
        { title, content },
        { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } }
      );
      setTitle("");
...
<div className="flex items-center space-x-4">
  ...
  <button
    ...
    className="bg-[#3b5998] text-white px-4 py-2 rounded-lg"
  >
    Send
  </button>
</div>
...
<div className="flex space-x-6 mt-3 text-[#3b5998] text-sm font-semibold">
