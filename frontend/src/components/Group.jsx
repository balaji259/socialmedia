...
useEffect(()=>{
  ...
  useEffect(() => {
    const fetchAdmins = async () => {
      try {
        const response = await fetch(`/group/${id}/admins`);
...
try {
  const res = await axios.get(`/group/${id}/recent-photos`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (let pair of formData.entries()) {
        ...
        const response = await fetch("/group/post", {
            method: "POST",
            body: formData,
...
const getAllPosts=async ()=>{
  try {
    ...
    const response = await axios.get(`/group/posts/${id}`);
    if (!response.ok) {
...
<nav className="bg-[#3b5998] h-12 fixed top-0 w-full shadow-md z-50">
      ...
      <div className="relative w-full h-52">
        <img
          src={group.coverPhoto || "https://via.placeholder.com/800x200"}
          alt="Cover"
          className="w-full h-full object-cover"
        />
        <img
          src={group.profilePicture || "https://via.placeholder.com/100"}
          alt="Profile"
